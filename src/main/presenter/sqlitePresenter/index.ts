...
import { MESSAGE_FIELDS_NO_ID, MessagesTable } from './tables/messages'
...
export class SQLitePresenter implements ISQLitePresenter {
  ...
  public async updateConversation(
    ...
    // 定义字段映射关系
    const fieldMappings = [
      { field: 'title', prop: data.title },
      { field: 'is_new', prop: data.is_new },
      ...(data.settings
        ? [
            { field: 'system_prompt', prop: data.settings.systemPrompt },
            { field: 'temperature', prop: data.settings.temperature },
            { field: 'context_length', prop: data.settings.contextLength },
            { field: 'max_tokens', prop: data.settings.maxTokens },
            { field: 'provider_id', prop: data.settings.providerId },
            { field: 'model_id', prop: data.settings.modelId },
            { field: 'artifacts', prop: data.settings.artifacts }
          ]
        : [])
    ]

    // 遍历映射关系添加更新字段
    fieldMappings.forEach(({ field, prop }) => {
      if (prop !== undefined) {
        updates.push(`${field} = ?`)
        params.push(prop)
      }
  ...
  public async updateMessage(
    ...
    // 定义字段映射关系
    const fieldMappings = [
      { field: 'content', prop: data.content },
      { field: 'status', prop: data.status },
      { field: 'metadata', prop: data.metadata },
      { field: 'is_context_edge', prop: data.isContextEdge },
      { field: 'token_count', prop: data.tokenCount }
    ]
    // 遍历映射关系添加更新字段
    fieldMappings.forEach(({ field, prop }) => {
      if (prop !== undefined) {
